...
import { Search, Download, FileText, Database, AlertTriangle, ShieldCheck, Lightbulb } from 'lucide-react'
import { listTemplates, installTemplate } from '../api/platform.js'
    ...
    <div data-testid="marketplace-page" className="p-8 max-w-[1400px]">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted text-sm mt-1">Install with one click. Customise prompts and tools after install.</p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="bg-elev border border-line rounded-md pl-9 pr-3 py-2 text-sm w-64 focus:border-accent outline-none"
          />
        </div>
      </div>
        ...
        {templates.map((tpl, i) => {
          ...
          return (
            <div
              ...
              className="rounded-xl border border-line bg-panel/60 backdrop-blur p-5 card-hover fade-up"
            ...
            >
              ...
              <div className="flex items-center gap-1.5 flex-wrap mb-4">
                {(tpl.suggested_tools || []).map(t => (
                  <span key={t} className="text-[10px] font-mono uppercase tracking-wide px-1.5 py-0.5 rounded border border-line text-muted bg-elev/50">{t}</span>
                ))}
                {tpl.hitl_enabled && (
                  <span className="text-[10px] font-mono uppercase tracking-wide px-1.5 py-0.5 rounded border border-warn/30 text-warn bg-warn/10">HITL</span>
                )}
              </div>
              <button
                data-testid={`install-btn-${tpl.template_id}`}
                onClick={() => install(tpl)}
                disabled={installing === tpl.template_id}
                className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-md bg-accent text-white text-sm font-medium hover:opacity-90 disabled:opacity-50"
              >
