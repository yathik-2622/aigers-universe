...
For the full end-user walkthrough — concepts, prompts, best practices, page-by-page reference, API reference, and troubleshooting — read **[USER_GUIDE.md](./USER_GUIDE.md)**.
...
> - **Tiger Analytics AI Gateway API key** (an OpenAI-compatible key)
...
### 4 · VS Code launch configuration (optional but nice)
...
And `.vscode/tasks.json` for the frontend:
```json
{
  "version": "2.0.0",
  "tasks": [
...
The full pytest suite created during the build lives at `backend/tests/test_platform_e2e.py`. Run it from the backend folder with the venv activated:
...
> The full suite needs a live `LLM_API_KEY` because it executes a real workflow. Use a test gateway or a low-quota key.
...
## Notes
- **Auth**: not implemented — open access by design for the MVP. Wrap with your IDP / SSO before production exposure.
...
- **GitHub push**: this repo uses Emergent's `Save to Github` button — invoke from the chat input to push to `github.com/yathik-2622/aigers-universe`.
